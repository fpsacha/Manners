...
Note also a **regression**: `CHANGELOG.md` 1.3.0 states the macro offers both the bare first name and the full name so the game can pick whichever resolves. **[SUPERSEDED -- do not act on this paragraph.** Both spellings were restored, produced a defect in each of the three rounds that followed, and were then removed deliberately: the full name resolves here, so the failure this asks to guard against has never been observed, while aiming at a bare first name can cast at, and speak at, a different player who shares it. The reasoning is in `Core.lua` above `ExpirePendingClick`.**] `Prompt.lua:743` emits only `/target entry.name`; `ns.FirstName` now survives solely in `ExpandTokens` (`Core.lua:1130`). The fix was lost in the 1.6.0 cleanup.
